...
import pandas as pd import os import tempfile from google.oauth2 import service_account from googleapiclient.discovery import build from googleapiclient.http import MediaFileUpload
...
if st.session_state.step == "login": with st.form("login_form"): note_number = st.text_input('رقم المذكرة', key="note_input") password = st.text_input('كلمة السر', type='password', key="pass_input") submitted = st.form_submit_button("✅ تأكيد")
...
elif st.session_state.step == "upload" and not st.session_state.upload_success: memo_info = st.session_state.memo_info st.success("✅ تم التحقق من المعلومات بنجاح") st.markdown(f""" ### 📄 عنوان المذكرة: {memo_info.get('عنوان المذكرة', 'غير متوفر')} ### 🎓 الطلبة: - {memo_info.get('الطالب الأول', '---')} {f"- {memo_info.get('الطالب الثاني')}" if pd.notna(memo_info.get('الطالب الثاني')) else ""} """)
...
if st.session_state.upload_success: st.success("✅ تم إيداع المذكرة بنجاح!") st.info(f"📎 معرف الملف على Drive: {st.session_state.file_id}") if st.button("⬅️ إنهاء"): for key in ["step", "validated", "upload_success", "file_id", "memo_info"]: st.session_state[key] = None if key == "memo_info" else False if key in ["validated", "upload_success"] else "login" st.experimental_rerun()
